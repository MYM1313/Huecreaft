...
import { ArrowLeft, CheckCircle2, ArrowRight, ShieldCheck, Sparkles, Ruler, Paintbrush, Search, ClipboardList, Hammer } from 'lucide-react';
...
const ServiceDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-luxury-gold/20 overflow-x-hidden pt-24">
      ...
      {/* Image Carousel Section: Premium 3D Swipeable Gallery */}
      <section className="py-6 md:py-10 bg-white overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <div className="relative">
            <div className="flex gap-5 md:gap-8 overflow-x-auto pb-10 snap-x snap-mandatory no-scrollbar touch-pan-y px-4">
              {service.gallery.map((img, i) => (
                <motion.div
                  ...
                  className="flex-none w-[85vw] md:w-[600px] aspect-[4/3] relative snap-center"
                >
                  {/* 3D Shadow Layer */}
                  <div className="absolute -inset-4 bg-luxury-gold/5 blur-3xl rounded-[3rem] -z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  
                  <div className="h-full w-full overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] border border-luxury-divider/20 bg-white relative group/img transition-all duration-500 group-hover:border-luxury-gold/30">
                    <motion.img
                      src={img}
                      alt={`Gallery ${i}`}
                    ...
                    {/* Premium Overlays */}
                    <div className="absolute inset-0 bg-gradient-to-t from-luxury-ink/50 via-transparent to-transparent opacity-0 group-hover/img:opacity-100 transition-opacity duration-700" />
                    <div className="absolute inset-0 ring-1 ring-inset ring-white/30 rounded-[2.5rem] md:rounded-[3.5rem]" />
                    
                    {/* Glass Shine Effect */}
                    <div className="absolute inset-0 opacity-0 group-hover/img:opacity-30 transition-opacity duration-1000 bg-gradient-to-tr from-transparent via-white/40 to-transparent -translate-x-full group-hover/img:translate-x-full transform duration-1000" />
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="flex justify-center gap-1.5 mt-2 md:hidden">
              {service.gallery.map((_, i) => (
                <div key={i} className="h-1 w-4 rounded-full bg-luxury-gold/20" />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid lg:grid-cols-5 gap-8 items-start">
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.8, ease: [0.4, 0, 0.2, 1] }}
              className="lg:col-span-2 bg-white p-6 md:p-8 rounded-2xl md:rounded-3xl border border-luxury-divider/50 space-y-6"
            >
